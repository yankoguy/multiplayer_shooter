...
class Tile(BaseObject):
    ...
    def __init__(self, color: tuple, map_x_index:int, map_y_index:int):
        super().__init__(map_x_index*TILE_WIDTH, map_y_index*TILE_HEIGHT, TILE_WIDTH, TILE_HEIGHT, color)
...
class Map(metaclass=Singleton):
    ...
    """
    _map_file = "D:\\Guy\\PycharmProjects\\multiplayer_game\\src\\game_world\\Map\\map.txt"
    _tile_symbols = {'0' : BLUE, '1' : GREEN}
    def __init__(self):
        self.tiles = []
        self._create_map()
    ...
    def _create_map(self):
        ...
        """
        with open(Map._map_file,'r') as tiles:
            for y_tile_index,line in enumerate(tiles):
                for x_tile_index, tile_symbol in enumerate(line.strip()):
                    self.tiles.append(Tile(Map._tile_symbols[tile_symbol],x_tile_index,y_tile_index))
